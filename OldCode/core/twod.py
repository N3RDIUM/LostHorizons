...
class TwoDTerrain:
    ...
    def generate_chunk(self, position):
        self.chunks[tuple(position)] = Node(
            rect=[
                (position[0] * self.chunksize, 0,
                 position[2] * self.chunksize),
                (
                    position[0] * self.chunksize + self.chunksize,
                    0,
                    position[2] * self.chunksize,
                ),
                (
                    position[0] * self.chunksize + self.chunksize,
                    0,
                    position[2] * self.chunksize + self.chunksize,
                ),
                (
                    position[0] * self.chunksize,
                    0,
                    position[2] * self.chunksize + self.chunksize,
                ),
            ],
    ...
    def update(self, player):
        ...
        currchunk = [
            -player.position[0] // self.chunksize,
            -player.position[2] // self.chunksize,
        ]
        chunks = []
        for x in range(-self.render_distance, self.render_distance):
            for z in range(-self.render_distance, self.render_distance):
