...
class Player(object):
    ...
    def update(self, window):
        ...
        self.speed = (((dist(self.position, self.planet.center) -
                        (self.planet.size / 10 * 9.99)) / self.planet.size) *
                      self.speed_mlt / 100000 * self.planet.size)
        ...
        if (self.planet.atmosphere["enabled"]
                and dist(self.position,
                         self.planet.center) < self.planet.atmosphere["end"]):
            glEnable(GL_FOG)
            ...
            mlt = 1 - ((dist(self.position, self.planet.center) -
                        self.planet.atmosphere["start"]) /
                       (self.planet.atmosphere["end"] -
                        self.planet.atmosphere["start"]))
