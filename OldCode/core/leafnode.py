...
class LeafNode:
    ...
    def generate(self):
        ...
        self._vertices = np.array([corner1, corner2, corner3, corner4],
                                  dtype=np.float32)
    ...
    def _tes(self):
        ...
        self._vertices = np.array(self.tesselate(self._vertices, times=1),
                                  dtype=np.float32)
        self.times_tessed += 1
    ...
    def _finalise_mesh(self):
        ...
        # Get the normals
        self._normals = self.get_normals(np.array(self._vertices))
    ...
    def tesselate(self, vertices, times=1):
        ...
        return np.array(self.tesselate(new_vertices, times - 1),
                        dtype=np.float32)
    ...
    def add_noise(self, vertices):
        ...
        for i in range(len(vertices)):
            vertices[i] = [
                vertices[i][0],
                vertices[i][1] +
                noise.snoise2(vertices[i][0] / 100, vertices[i][2] / 100) * 32,
                vertices[i][2],
            ]
        return vertices
    ...
    def spherify_and_add_noise(self, vertices):
        ...
        for i in range(len(vertices)):
            ...
            _noise = (self.fractal_noise(
                (x * vector[0], y * vector[1], z * vector[2])) * 100)
            x = x + vector[0] * _noise
            y = y + vector[1] * _noise
            z = z + vector[2] * _noise
    ...
    def fractal_noise(self, coords):
        ...
        for i in range(12):  # TODO: Add a config value named "octaves"
            noiseSum += (
                noise.snoise3(x * frequency, y * frequency, z * frequency) *
                amplitude)
            frequency *= 2
            amplitude /= 2
