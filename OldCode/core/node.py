...
class Node:
    ...
    def __init__(
        self,
        rect=[(0, 0, 0), (100, 0, 0), (100, 0, 100), (0, 0, 100)],
        level=1,
        parent=None,
        planet=None,
        tokill=None,
        ...
        self._children = []
        self.position = [
            (self.rect[0][0] + self.rect[1][0] + self.rect[2][0] +
             self.rect[3][0]) / 4,
            (self.rect[0][1] + self.rect[1][1] + self.rect[2][1] +
             self.rect[3][1]) / 4,
            (self.rect[0][2] + self.rect[1][2] + self.rect[2][2] +
             self.rect[3][2]) / 4,
        ]
        self.size = (math.dist(rect[0], self.position) + math.dist(
            rect[1], self.position) + math.dist(rect[2], self.position) +
            math.dist(rect[3], self.position)) / 4
    ...
    def generate_split(self):
        ...
        node1 = Node(rect=rect1,
                     level=self.level + 1,
                     parent=self,
                     planet=self.planet)
        node2 = Node(rect=rect2,
                     level=self.level + 1,
                     parent=self,
                     planet=self.planet)
        node3 = Node(rect=rect3,
                     level=self.level + 1,
                     parent=self,
                     planet=self.planet)
        node4 = Node(rect=rect4,
                     level=self.level + 1,
                     parent=self,
                     planet=self.planet)
        ...
        if self.generated:
            ...
            try:
                if self.tokill:
                    ...
                    self.tokill = None
                    try:
                        del self.planet.chunks[idx]
                    except:
                        pass
                    try:
                        del self.planet.chunks[pidx]
                    except:
                        pass
            except:
                pass
