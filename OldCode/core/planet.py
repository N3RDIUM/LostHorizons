...
class DummyPlanet:
    ...
    def __init__(
        ...
        self.center = [
            self.center[0] + self.position[0],
            self.center[1] + self.position[1],
            self.center[2] + self.position[2],
        ]
...
class Planet:
    ...
    def generate(self):
        ...
        self.generate_chunk("top", [(0, size, 0), (0, size, size),
                                    (size, size, size), (size, size, 0)])
        self.generate_chunk("left",
                            rect=[(0, 0, 0), (0, 0, size), (0, size, size),
                                  (0, size, 0)])
        self.generate_chunk("front",
                            rect=[(0, 0, 0), (0, size, 0), (size, size, 0),
                                  (size, 0, 0)])

        self.generate_chunk("bottom",
                            rect=[(0, 0, 0), (size, 0, 0), (size, 0, size),
                                  (0, 0, size)])
        self.generate_chunk(
            "right",
            rect=[(size, 0, 0), (size, size, 0), (size, size, size),
                  (size, 0, size)],
        )
        self.generate_chunk(
            "back",
            rect=[(0, 0, size), (size, 0, size), (size, size, size),
                  (0, size, size)],
        )
    ...
    def update(self, player):
        ...
        self.rotation_details["current"][0] += self.rotation_details["speed"][
            0]
        self.rotation_details["current"][1] += self.rotation_details["speed"][
            1]
        self.rotation_details["current"][2] += self.rotation_details["speed"][
            2]
        ...
        if self.last_update_stage <= 2:
            for i in range(len(self.split_queue) + len(self.unify_queue)):
                ...
                tosplit = self.split_queue.pop(0) if len(
                    self.split_queue) > 0 else None
                if tosplit:
                    tosplit.swap_children()
                    tosplit.generate_split()

                # Process the unify queue
                tounify = self.unify_queue.pop(0) if len(
                    self.unify_queue) > 0 else None
                if tounify:
