...
class Simulation:
    ...
    def __init__(self, window):
        ...
        self.sphere = Sphere(self, 16, [0, 0, 0])
    ...
    def process(namespace):
        ...
        helpers = {"tesselator": fast_tesselate}
    ...
    def handleQueueItem(item, namespace, helpers):
        ...
        if item["task"] == "tesselate":
            ...
            result_buffer = shared_memory.SharedMemory(
                create=True, size=result.nbytes, name=str(uuid4())
            )
            shared_result = np.ndarray(
                result.shape, dtype=result.dtype, buffer=result_buffer.buf
            )
